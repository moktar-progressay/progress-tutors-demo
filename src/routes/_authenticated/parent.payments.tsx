...
export const Route = createFileRoute("/_authenticated/parent/payments")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Your plan, what is due next and every payment the office has recorded.",
      },
...
function ParentPayments() {
  ...
  const outstanding = myPayments.filter((p) => p.status !== "received");
  const perCycle = mySubs
  ...
  return (
    <Page>
      <PageHeader
        title="My payments"
        subtitle="View subscriptions, invoices and secure payment links from the office."
      />
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <StatCard
          label="Amount due"
          value={money(outstanding.reduce((a, p) => a + num(p.amount), 0))}
          tone="amber"
        />
        <StatCard
          label="Active plans"
          ...
          tone="purple"
        />
        <StatCard label="Per billing cycle" value={money(perCycle)} tone="blue" />
      </div>
      ...
      <Section id="parent-history" title="Invoices and payments">
        {myPayments.length === 0 ? (
            ...
            {myPayments.map((p) => (
              ...
              >
                <span className="min-w-0 flex-1 text-sm">
                  {prettyDate(
                    p.status === "received" ? p.payment_date : (p.due_date ?? p.payment_date),
                  )}{" "}
                  · {childName(p.student_id)}
                </span>
                <span className="text-xs text-muted-foreground">{p.method ?? "Not recorded"}</span>
                <span className="font-bold">{money(p.amount)}</span>
                <Pill tone={p.status === "received" ? "green" : "amber"}>
                  {p.status === "received" ? "Paid" : "Payment due"}
                </Pill>
                {p.status !== "received" && p.payment_link ? (
                  <Button size="sm" asChild>
                    <a href={p.payment_link} target="_blank" rel="noreferrer">
                      <ExternalLink className="h-4 w-4" /> Pay securely
                    </a>
                  </Button>
                ) : null}
              </li>
