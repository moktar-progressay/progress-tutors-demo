...
export function FamilyInvoiceDialog({
  ...
  return (
    <FormDialog
      ...
      fullScreen
    >
      <div className="grid gap-5 sm:col-span-full lg:grid-cols-[minmax(0,1fr)_380px]">
        <div className="grid content-start gap-3 sm:grid-cols-2">
          <div className="sm:col-span-2">
            <p className="text-sm font-extrabold">Client & students</p>
            <p className="text-xs text-muted-foreground">
              Choose the billing contact, then add one or more linked students.
            </p>
          </div>
          <SelectField
        ...
        <aside className="h-fit rounded-2xl border border-border bg-muted/40 p-3 lg:sticky lg:top-0">
          <div className="mb-3 flex items-center justify-between">
            <div>
              <p className="text-sm font-extrabold">Invoice preview</p>
              <p className="text-[11px] text-muted-foreground">Updates as you type</p>
            </div>
            <Pill tone="green">Autosave on</Pill>
          </div>
          <div className="rounded-xl border border-border bg-white p-5 text-slate-900 shadow-sm">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-lg font-black">
                  Progress<span className="text-pink-600">Tutors</span>
                </p>
                <p className="mt-1 text-[10px] text-slate-500">Progressay</p>
              </div>
              <div className="text-right">
                <p className="text-lg font-black">Invoice</p>
                <p className="text-xs text-slate-500">New draft</p>
              </div>
            </div>
            <div className="mt-6 grid grid-cols-2 gap-4 text-xs">
              <div>
                ...
                <p className="mt-1 text-slate-600">
                  {fullName(parents.find((parent) => parent.id === parentId)) || "Choose a client"}
                </p>
              </div>
              <div className="text-right text-slate-600">
                <p>Service: {periodStart || "Not set"}</p>
                <p>Due: {dueDate || "Not set"}</p>
              </div>
            </div>
            <div className="mt-6 overflow-hidden rounded-lg border border-slate-200">
              <div className="grid grid-cols-[1fr_35px_65px_70px] bg-slate-100 px-2 py-2 text-[10px] font-bold">
              ...
              {lines.length === 0 ? (
                ...
                lines.map((line) => (
                  <div
                    key={line.key}
                    className="grid grid-cols-[1fr_35px_65px_70px] border-t border-slate-100 px-2 py-2 text-[10px]"
                  >
                    <span className="pr-2 font-semibold">
                      {studentNamePreview(students, line.student_id)} ·{" "}
                      {line.description || "Service"}
                    </span>
                    <span>{line.quantity || "0"}</span>
                    <span>{money(num(line.unit_price))}</span>
            ...
            <div className="mt-5 ml-auto grid max-w-[230px] grid-cols-2 gap-y-2 text-xs">
              <span className="text-slate-500">Subtotal</span>
              <span className="text-right">{money(subtotal)}</span>
              <span className="text-slate-500">Discounts</span>
              <span className="text-right">-{money(discount)}</span>
              <span className="border-t border-slate-200 pt-2 font-black">Total</span>
              <span className="border-t border-slate-200 pt-2 text-right text-base font-black">
                {money(total)}
              </span>
              <span className="rounded-l-md bg-pink-50 px-2 py-2 font-bold text-pink-700">
                Amount due
              </span>
              <span className="rounded-r-md bg-pink-50 px-2 py-2 text-right font-black text-pink-700">
                {money(total)}
              </span>
            </div>
          </div>
...
function studentNamePreview(students: StudentRow[], id: string) {
  return fullName(students.find((student) => student.id === id)) || "Student";
}
